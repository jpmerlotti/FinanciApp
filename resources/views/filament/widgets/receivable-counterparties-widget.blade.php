<x-filament-widgets::widget>
    <x-filament::section>
        <x-slot name="heading">
            Contas a Receber (Por Pessoa/Empresa)
        </x-slot>

        @if(empty($counterparties))
            <div class="p-4 text-center text-gray-500 dark:text-gray-400">
                Nenhuma conta a receber pendente neste período.
            </div>
        @else
                ...
                @foreach($counterparties as $group)
                    <div x-data="{ open: false }" class="border rounded-lg dark:border-gray-700 overflow-hidden">
                        ...
                        <div @click="open = !open"
                            ...
                            <div class="flex items-center gap-2">
                                <span
                                    class="font-bold text-lg {{ $group['has_overdue'] ? 'text-danger-600' : '' }} text-success-600">
