...
class DateInputColumn extends TextInputColumn implements HasEmbeddedView
{
    protected Closure | string | null $type = 'date'; 
    ...
    protected function getFormattedState(): ?string
    {
        ...
        if ($state instanceof DateTimeInterface) {
            $format = $this->getType() === 'datetime-local' ? 'Y-m-d\TH:i' : 'Y-m-d';
    ...
    public function updateState(mixed $state): mixed
    {
        ...
        Notification::make()
            ...
            ->body('O vencimento foi atualizado com sucesso.')
            ->send();
