...
class ReceivableCounterpartiesWidget extends Widget
{
    use InteractsWithPageFilters;

    protected string $view = 'filament.widgets.receivable-counterparties-widget';
    ...
    public function getViewData(): array
    {
        ...
        // 1. Get Income (Receivables) that are Pending or Overdue
        // Grouped by Counterparty (Recipient)
        $transactions = Transaction::query()
            ->with(['counterParty', 'category'])
            ->where('type', TransactionTypes::INCOME->value)
            // Ideally we want PENDING or OVERDUE (Not Received Yet)
